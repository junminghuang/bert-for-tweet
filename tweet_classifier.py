...
# modified for virus tweet project. Jul 2020
#
...
class tweet_pairwise_processor(DataProcessor):
  """Processor for the MRPC data set (GLUE version)."""
...
def main(_):
  ...
  processors = {
      ...
      "mrpc": MrpcProcessor,
      "xnli": XnliProcessor,
      'tweet-pairwise': tweet_pairwise_processor,
      'tweet-pointwise': tweet_pointwise_processor,
  }
  ...
  if FLAGS.do_eval:
    ...
    output_eval_file = os.path.join(FLAGS.data_dir, "eval_results.txt")
...
if __name__ == "__main__":
  ...
  formatter = logging.Formatter('[%(asctime)s][%(levelname)s][%(filename)s:%(lineno)s] %(message)s',
                                '%Y-%b-%d %H:%M:%S')
  logger_info_handler.setFormatter(formatter)  # create a logging format
